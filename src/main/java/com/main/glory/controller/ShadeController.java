...
public class ShadeController extends ControllerConfig {
	...
	public ResponseEntity<GeneralResponse<Boolean,Object>> addShadeData(@RequestBody AddShadeMast shadeMast ,@RequestHeader Map<String, String> headers){
		...
		try {
			if(shadeMast == null){
				result =  new GeneralResponse<>(false, Constant.Null_Record_Passed, false, System.currentTimeMillis(), HttpStatus.OK,shadeMast);
			}else {
				shadeService.saveShade(shadeMast,headers.get("id"));
				result = new GeneralResponse<>(true, Constant.Shade_Added, true, System.currentTimeMillis(), HttpStatus.OK,shadeMast);
			}
	...
	public GeneralResponse<GetAPC,Object> getApcNumber(){
		try {
			...
			return new GeneralResponse<>(acp, Constant.Shade_Found, true, System.currentTimeMillis(), HttpStatus.CREATED,request.getRequestURI());
	...
	public ResponseEntity<GeneralResponse<Boolean,Object>> isAPCExist(@PathVariable(name = "number")String number){
		...
		try {
			...
			if(acp)
				result = new GeneralResponse<>(acp, Constant.Shade_Not_Found, false, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
			else
				result = new GeneralResponse<>(acp, Constant.Shade_Found, true, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
			...
			switch (getBy) {
				case "own":
					...
					if(!shadeMast.isEmpty())
						result = new GeneralResponse<>(shadeMast, Constant.Shade_Found, true, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
					else
						result = new GeneralResponse<>(null, Constant.Shade_Not_Found, false, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
	...
	public ResponseEntity<GeneralResponse<ShadeMast,Object>> getShadesById(@PathVariable(value = "id") Long id){
		...
		try{
			...
			if(shadeMast != null){
				result = new GeneralResponse<>(shadeMast.get(), Constant.Shade_Found, true, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
			}else{
				result = new GeneralResponse<>(null, Constant.Shade_Not_Found, false, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
			}
			logService.saveLog(result,request,debugAll);
	...
	public ResponseEntity<GeneralResponse<List<GetShadeByPartyAndQuality>,Object>> getShadesByQualityAndPartyId(@PathVariable(value = "qualityId") String qualityId,@PathVariable(value = "partyId") String partyId,@RequestHeader Map<String, String> headers){
		...
		try{
			...
			if(shadeMastList != null){
				result= new GeneralResponse<>(shadeMastList, Constant.Shade_Found, true, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
			}else{
				result= new GeneralResponse<>(null, Constant.Shade_Not_Found, false, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
			}
			logService.saveLog(result,request,debugAll);
	...
	public ResponseEntity<GeneralResponse<Boolean,Object>> updateShadeById(@RequestBody ShadeMast shadeMast) throws Exception {
		...
		try {
			if (shadeMast.getId() != null) {
				...
				if (flag) {
					result = new GeneralResponse<>(true, Constant.Shade_Updated, true, System.currentTimeMillis(), HttpStatus.OK, shadeMast);
				} else {
					result = new GeneralResponse<>(false, Constant.Shade_Not_Found, false, System.currentTimeMillis(), HttpStatus.OK, shadeMast);
				}
				logService.saveLog(result, request, debugAll);
			} else
	...
	public ResponseEntity<GeneralResponse<Boolean,Object>> deleteshadeById(@PathVariable(value = "id") Long id) {
		...
		try{
			...
			result= new GeneralResponse<>(true, Constant.Shade_Deleted, true, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
			logService.saveLog(result,request,debugAll);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ResponseEntity<GeneralResponse<List<GetShadeByPartyAndQuality>,Object>> getShadeByPartyAndWithAndWithoutQuality(@RequestParam(name = "partyId") Long partyId, @RequestParam(name = "qualityId")Long qualityId) throws Exception {
		...
		try {
			if (partyId != null) {
				...
				if (!flag.isEmpty()) {
					result =  new GeneralResponse<>(flag, Constant.Shade_Found, true, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
				} else {
					result= new GeneralResponse<>(null, Constant.Shade_Not_Found, false, System.currentTimeMillis(), HttpStatus.OK,request.getRequestURI());
				}
			}
