...
public interface TaskDataDao extends JpaRepository<TaskData,Long> {
    ...
    @Query("select new com.main.glory.model.task.request.TaskDetail(t,(select tt from TaskMast tt where tt.id=t.controlId) as taskMast) from TaskData t where t.taskStatus=:status AND t.assignUserId=:userId OR t.controlId IN(select tm.id from TaskMast tm.createdBy=:userId)")
