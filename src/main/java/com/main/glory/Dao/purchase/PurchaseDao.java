...
public interface PurchaseDao extends JpaRepository<Purchase,Long> {
    ...
    @Query("select new com.main.glory.model.purchase.response.PurchaseResponse(p,(select d.name from Department d where d.id=p.departmentId) as deptName,(select d.name from Authorize d where d.id=p.receiverById) as rname,(select d.name from Authorize d where d.id=p.AuthorizeId) as aname) from Purchase p")
    List<PurchaseResponse> getAllPurchaseRecord();

    @Query("select new com.main.glory.model.purchase.response.PurchaseResponse(p,(select d.name from Department d where d.id=p.departmentId) as deptName,(select d.name from Authorize d where d.id=p.receiverById) as rname,(select d.name from Authorize d where d.id=p.AuthorizeId) as aname) from Purchase p where p.id=:id")
