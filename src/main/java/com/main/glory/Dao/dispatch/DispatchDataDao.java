...
public interface DispatchDataDao extends JpaRepository<DispatchData, Long> {
    ...
    @Query("select new com.main.glory.model.dispatch.response.GetBatchByInvoice(SUM(dd.batchData.id) as batch,dd.batchId,dd.stockId) from DispatchData dd where dd.dispatchMast.postfix=:invoiceNo GROUP BY dd.batchId,dd.stockId")
    ...
    @Modifying
    @Transactional
    @Query("update DispatchData dd set dd.isSendToParty=true where dd.dispatchMast.postfix=:invoiceNumberExist")
    void updateStatus(String invoiceNumberExist);


    //for bill with or without bill send to party
    @Query("select new com.main.glory.model.dispatch.response.GetBatchByInvoice(SUM(dd.batchData.id) as batch,dd.batchId,dd.stockId) from DispatchData dd where dd.dispatchMast.postfix=:invoiceExist GROUP BY dd.batchId,dd.stockId")
    ...
    @Query("select new com.main.glory.model.dispatch.response.GetBatchByInvoice(COUNT(d.batchData.id) as batchEntryId,d.batchId as batchId,d.stockId as stockId) from DispatchData d where d.dispatchMast.postfix=:invoiceNumber GROUP BY d.batchId,d.stockId")
    List<GetBatchByInvoice> getAllStockByInvoiceNumber(String invoiceNumber);

    @Query("select new com.main.glory.model.dispatch.response.QualityWithRateAndTotalMtr(x.quality.id,(select q.qualityName from QualityName q where q.id = (select qq.qualityName.id from Quality qq where qq.id=x.quality.id)),(select q.qualityId from Quality q where q.id=x.quality.id),x.qualityRate) from DispatchData x where x.dispatchMast.postfix=:invoiceNo GROUP BY x.quality.id")
    List<QualityWithRateAndTotalMtr> getAllQualityByInvoiceNo(String invoiceNo);

    @Query("select (x.batchData.id) from DispatchData x where x.dispatchMast.postfix=:invoiceNo AND x.quality.id=:qualityEntryId")
    List<Long> getAllBatchEntryIdByQualityAndInvoice(Long qualityEntryId, String invoiceNo);

    @Query("select x.batchData.id from DispatchData x where x.dispatchMast.postfix=:invoiceNo")
...
*/
    @Query("select x.qualityRate from DispatchData x where x.batchData.id=:id AND x.dispatchMast.postfix=:invoiceNumber")
    Double getQualityRateByInvoiceAndBatchEntryId(String invoiceNumber, Long id);


    @Modifying
    @Transactional
    @Query("update DispatchData x set x.qualityRate=:rate where x.batchId=:batchId AND x.dispatchMast.postfix=:invoiceNo")
    void updateQualityRateWithBatchIdAndInvoiceNo(String invoiceNo, String batchId, Double rate);

    @Query("select x.billingUnit from DispatchData x where x.batchData.id=:id AND x.dispatchMast.postfix=:invoiceNumber")
    String getBillingUnitByInvoiceAndBatchEntryId(String invoiceNumber, Long id);

    @Query("select x.wtPer100m from DispatchData x where x.batchData.id=:id AND x.dispatchMast.postfix=:invoiceNumber")
    Double getWtPer100mByInvoiceAndBatchEntryId(String invoiceNumber, Long id);

    @Modifying
    @Transactional
    @Query("update DispatchData x set x.qualityRate=:rate where x.pchallanRef=:pchallanRef AND x.dispatchMast.postfix=:invoiceNo")
    void updateQualityRateWithPChallanRefAndInvoiceNo(String invoiceNo, String pchallanRef, Double rate);


    @Query("select new com.main.glory.model.dispatch.response.GetBatchByInvoice(SUM(dd.batchData.id) as batch,dd.stockId,dd.pchallanRef) from DispatchData dd where dd.dispatchMast.postfix=:invoiceExist GROUP BY dd.pchallanRef,dd.stockId")
    ...
    @Query("select new com.main.glory.model.dispatch.response.UnitDetail(d.billingUnit,d.inwardUnit,d.wtPer100m) from DispatchData d where d.dispatchMast.postfix=:invoiceNo AND d.batchData.id=:batchEntryId")
    ...
    @Query("select new com.main.glory.model.dispatch.response.GetBatchByInvoice(SUM(dd.batchData.id) as batch,dd.stockId,dd.pchallanRef,dd.batchId) from DispatchData dd where dd.dispatchMast.postfix=:invoiceExist GROUP BY dd.pchallanRef,dd.stockId,dd.batchId")
