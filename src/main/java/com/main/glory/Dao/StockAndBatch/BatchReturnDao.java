...
public interface BatchReturnDao extends JpaRepository<BatchReturn,Long> {
    ...
    @Query("select new com.main.glory.model.StockDataBatchData.response.ReturnBatchDetail(x.id,x.partyId,x.partyName,x.address,x.partyCode,x.qualityEntryId,x.qualityName,x.qualityId,SUM(x.mtr),x.chlNo,x.challanDate) from BatchReturn x where x.chlNo=:chlNo")
    ReturnBatchDetail getChallanDetailByChlNo(Long chlNo);
}
