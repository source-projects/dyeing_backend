...
*/
    ...
    */
    ...
    @Query("select new com.main.glory.model.StockDataBatchData.response.PendingBatchData(x.batchId,x.pchallanRef,SUM(x.mtr),SUM(x.wt),(select s.receiveDate from StockMast s where s.id=:id)) from BatchData x where x.controlId=:id AND x.isBillGenrated=false GROUP BY x.batchId,x.pchallanRef")
