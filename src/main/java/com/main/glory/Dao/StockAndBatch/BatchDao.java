...
*/
    ...
    */
    ...
    @Query("select new com.main.glory.model.StockDataBatchData.response.GetBatchWithControlId(p.batchId as batchId,p.controlId as controlId,SUM(p.wt) as WT,SUM(p.mtr) as MTR) from BatchData p where p.batchId IS NOT NULL AND p.controlId IS NOT NULL AND p.mergeBatchId IS NOT NULL AND p.mergeBatchId =:mergeBatchId GROUP BY p.batchId,p.controlId ")
...
//    List<PendingBatchDataForExcel> getPendingBatchListForExcelByStockId(Long id);
    ...
    @Query("select new com.main.glory.model.StockDataBatchData.response.GetBatchWithControlId(p.mergeBatchId,SUM(p.wt) as WT,SUM(p.mtr) as MTR) from BatchData p INNER JOIN StockMast sm on sm.id=p.controlId where (:batchId IS NULL OR p.batchId LIKE %:batchId%) AND (:mergeBatchId IS NULL OR p.mergeBatchId LIKE %:mergeBatchId%) AND (:partyId IS NULL OR sm.party.id=:partyId) AND (:qualityEntryId IS NULL OR sm.quality.id=:qualityEntryId) AND p.batchId IS NOT NULL AND p.mergeBatchId IS NOT NULL AND p.controlId IS NOT NULL GROUP BY p.mergeBatchId")
    List<GetBatchWithControlId> findAllMergeBatchWithFilter( Long partyId, Long qualityEntryId, String batchId, String mergeBatchId, Pageable pageable);
    ...
    List<GetBatchWithControlId> findAllMergeBatchWithFilterWithAndTrueWithPageble(String partyName, String qualityName, String batchId, String mergeBatchId, Pageable pageable);

    @Query("select new com.main.glory.model.StockDataBatchData.response.GetBatchWithControlId(p.mergeBatchId,SUM(p.wt) as WT,SUM(p.mtr) as MTR) from BatchData p INNER JOIN StockMast sm on sm.id=p.controlId where (:mergeBatchId IS NULL OR p.mergeBatchId = :mergeBatchId) AND (:batchId IS NULL OR p.batchId = :batchId) AND (:partyName IS NULL OR sm.party.partyName LIKE %:partyName%) AND (:qualityName IS NULL OR sm.quality.qualityName.qualityName LIKE %:qualityName%) AND p.batchId IS NOT NULL AND p.mergeBatchId IS NOT NULL AND p.controlId IS NOT NULL GROUP BY p.mergeBatchId")
    List<GetBatchWithControlId> findAllMergeBatchWithFilterWithAndTrue(String partyName, String qualityName, String batchId, String mergeBatchId);


    @Query("select new com.main.glory.model.StockDataBatchData.response.GetBatchWithControlId(b.mergeBatchId,SUM(b.wt) as WT,SUM(b.mtr) as MTR) from BatchData b INNER JOIN StockMast sm on sm.id=b.controlId  where ((:mergeBatchId IS NULL OR b.mergeBatchId = :mergeBatchId) OR (:batchId IS NULL OR b.batchId = :batchId) OR (:partyName IS NULL OR sm.party.partyName LIKE %:partyName%) OR (:qualityName IS NULL OR sm.quality.qualityName.qualityName LIKE %:qualityName%)) AND b.batchId IS NOT NULL AND b.mergeBatchId IS NOT NULL AND b.controlId IS NOT NULL GROUP BY b.mergeBatchId")
    List<GetBatchWithControlId> findAllMergeBatchWithFilterWithAndFalseWithPageble(String partyName, String qualityName, String batchId, String mergeBatchId, Pageable pageable);

    @Query("select new com.main.glory.model.StockDataBatchData.response.GetBatchWithControlId(b.mergeBatchId,SUM(b.wt) as WT,SUM(b.mtr) as MTR) from BatchData b INNER JOIN StockMast sm on sm.id=b.controlId  where ((:mergeBatchId IS NULL OR b.mergeBatchId = :mergeBatchId) OR (:batchId IS NULL OR b.batchId = :batchId) OR (:partyName IS NULL OR sm.party.partyName LIKE %:partyName%) OR (:qualityName IS NULL OR sm.quality.qualityName.qualityName LIKE %:qualityName%)) AND b.batchId IS NOT NULL AND b.mergeBatchId IS NOT NULL AND b.controlId IS NOT NULL GROUP BY b.mergeBatchId")
    List<GetBatchWithControlId> findAllMergeBatchWithFilterWithAndFalse(String partyName, String qualityName, String batchId, String mergeBatchId);
   /* @Query(value = "select new com.main.glory.model.StockDataBatchData.response.GetBatchWithControlId(b.merge_batch_id,SUM(b.wt) as WT,SUM(b.mtr) as MTR) from batch_data as b INNER JOIN stock_mast as sm on sm.id=b.control_id inner join party as p on p.id=sm.party_id inner join quality as q on q.id=sm.quality_id inner join quality_name as qn on qn.id=q.quality_name_id where ((:mergeBatchId IS NULL OR b.merge_batch_id = :mergeBatchId) OR (:batchId IS NULL OR b.batch_id = :batchId) OR (:partyName IS NULL OR p.party_name LIKE %:partyName%) OR (:qualityName IS NULL OR qn.quality_name LIKE %:qualityName%)) AND b.batch_id IS NOT NULL AND b.merge_batch_id IS NOT NULL AND b.control_id IS NOT NULL GROUP BY b.merge_batch_id",nativeQuery = true)
    List<Object[]> findAllMergeBatchWithFilterWithAndFalseWithPageble(@Param("partyName") String partyName,@Param("qualityName") String qualityName,@Param("batchId") String batchId, @Param("mergeBatchId") String mergeBatchId, Pageable pageable);


    @Query(value = "select new com.main.glory.model.StockDataBatchData.response.GetBatchWithControlId(b.merge_batch_id,SUM(b.wt) as WT,SUM(b.mtr) as MTR) from batch_data as b INNER JOIN stock_mast as sm on sm.id=b.control_id inner join party as p on p.id=sm.party_id inner join quality as q on q.id=sm.quality_id inner join quality_name as qn on qn.id=q.quality_name_id where ((:mergeBatchId IS NULL OR b.merge_batch_id = :mergeBatchId) OR (:batchId IS NULL OR b.batch_id = :batchId) OR (:partyName IS NULL OR p.party_name LIKE %:partyName%) OR (:qualityName IS NULL OR qn.quality_name LIKE %:qualityName%)) AND b.batch_id IS NOT NULL AND b.merge_batch_id IS NOT NULL AND b.control_id IS NOT NULL GROUP BY b.merge_batch_id",nativeQuery = true)
    List<Object[]> findAllMergeBatchWithFilterWithAndFalse(String partyName, String qualityName, String batchId, String mergeBatchId);
*/

    @Query("select x.controlId from BatchData x where x.mergeBatchId=:mergeBatchId GROUP BY x.controlId")
    List<Long> getStockMastByMergeBatchId(String mergeBatchId);
    ...
    @Query("select new com.main.glory.model.StockDataBatchData.response.FabricInV2Data(b.batchId,b.pchallanRef,s.quality.qualityName.qualityName,s.quality.qualityId,count(b.id),SUM(b.mtr),SUM(b.wt),s.party.partyName,s.party.partyCode,s.quality.rate,(s.quality.rate * SUM(b.mtr) * 0.9) as billingValues) from BatchData b INNER JOIN StockMast s on b.controlId = s.id where b.controlId=:id AND b.isExtra=false GROUP BY b.batchId,b.pchallanRef")
    List<FabricInV2Data> getBatchListForFabricDataByStockIdWithoutExtraAndFilter(Long id);

    @Query("select new com.main.glory.model.StockDataBatchData.response.FabricInData(b.batchId,b.pchallanRef,s.quality.qualityName.qualityName,s.quality.qualityId,count(b.id),SUM(b.mtr),SUM(b.wt),s.quality.rate,(s.quality.rate * SUM(b.mtr) * 0.9) as billingValues) from BatchData b INNER JOIN StockMast s on b.controlId = s.id where b.controlId=:id AND b.isExtra=false GROUP BY b.batchId,b.pchallanRef")
    List<FabricInData> getBatchListByStockIdWithoutExtraAndFilter(Long id);

    @Query("select new com.main.glory.model.StockDataBatchData.response.BatchWithTotalMTRandFinishMTR(p.batchId as batchId,p.pchallanRef,p.controlId as controlId,SUM(p.wt) as WT,SUM(p.mtr) as MTR,SUM(p.finishMtr) as finishMtr,count(p.id) as count,sm) from BatchData p inner join StockMast sm on sm.party.id =:partyId where p.controlId=sm.id AND sm.isRfInvoice =:rfInvoiceFlag AND p.isProductionPlanned = true AND p.isFinishMtrSave=true AND p.isBillGenrated=false GROUP BY p.batchId,p.controlId,p.pchallanRef")
    List<BatchWithTotalMTRandFinishMTR> getAllPChallanByPartyIdAndRfInvoiceFlagWithTotalFinishMtr(Long partyId, Boolean rfInvoiceFlag);
    ...
    List<FabricInData> getBatchListWithPartyAndMasterByStockIdWithoutExtraAndFilter(Date from, Date to, Long partyId, Long qualityEntryId, Long userHeadId, Long qualityNameId);

    @Query("select new com.main.glory.model.StockDataBatchData.response.FabricInDetailsChildData(b.batchId,s.quality.qualityName.qualityName,s.quality.qualityId,count(b.id),SUM(b.mtr),SUM(b.wt),s.quality.rate,(s.quality.rate * CASE s.quality.billingUnit WHEN 'weight' THEN SUM(b.wt) ELSE SUM(b.mtr) END * 0.9) as billingValues,s.party,s.party.userHeadData,s.quality) from BatchData b INNER JOIN StockMast s on b.controlId = s.id  where b.avoidCommission=false AND (:from IS NULL OR DATE(s.createdDate)>=DATE(:from)) AND (:to IS NULL OR DATE(s.createdDate)<=DATE(:to)) AND (:partyId IS NULL OR s.party.id=:partyId) AND (:qualityNameId IS NULL OR s.quality.qualityName.id=:qualityNameId) AND (:qualityEntryId IS NULL OR s.quality.id=:qualityEntryId) AND (:userHeadId IS NULL OR s.party.userHeadData.id=:userHeadId) AND b.batchId IS NOT NULL AND b.isExtra=false GROUP BY b.batchId")
